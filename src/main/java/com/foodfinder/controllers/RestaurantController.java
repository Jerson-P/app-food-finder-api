...
public class RestaurantController {
	...
	@PostMapping
	public ResponseEntity<ResponseDTO> save(@RequestBody RestaurantDTO restaurant){
		return this.RestaurantService.saveRestaurant(restaurant);
	}
	
	@Operation(summary = "Operación que permite eliminar los registros del restaurante por id")
	@ApiResponses(value = {
	...
	@Operation(summary = "Operación que permite actualizar el restaurnate a partir de un id")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Se consulta exitosamente", content = {
			@Content(mediaType = "application/json", schema = @Schema(implementation = com.foodfinder.dtos.ResponseDTO.class)) }),
			@ApiResponse(responseCode = "400", description = "La petición no puede ser entendida por el servidor debido a errores de sintaxis, el cliente no debe repetirla no sin antes hacer modificaciones", content = {
					@Content(mediaType = "application/json", schema = @Schema(implementation = ResponseDTO.class)) }),
	...
	@PutMapping("/{id}")
    public ResponseEntity<ResponseDTO> update(@PathVariable Integer id, @RequestBody RestaurantDTO restaurantDTO) {
