...
public class MenuController {
	...
	@PostMapping
	public ResponseEntity<ResponseDTO> save(@RequestBody MenuDTO menu){
		return this.menuServiceImpl.save(menu);
	}
	
	@Operation(summary = "Operación que permite eliminar los registros del menu")
	@ApiResponses(value = {
    ...
    public ResponseEntity<ResponseDTO> delete(@PathVariable Integer id) {
            return this.menuServiceImpl.delete(id);
    }
	
	@Operation(summary = "Operación que permite consultar un menú a partir de un id")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Se consulta exitosamente", content = {
			@Content(mediaType = "application/json", schema = @Schema(implementation = com.foodfinder.dtos.ResponseDTO.class)) }),
			@ApiResponse(responseCode = "400", description = "La petición no puede ser entendida por el servidor debido a errores de sintaxis, el cliente no debe repetirla no sin antes hacer modificaciones", content = {
					@Content(mediaType = "application/json", schema = @Schema(implementation = ResponseDTO.class)) }),
	...
	public ResponseEntity<ResponseDTO> getMenuId(@PathVariable Integer id) {
		return this.menuServiceImpl.findMenuById(id);
	}
	
	@Operation(summary = "Operación que permite actualizar una el menú a partir de un id")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Se consulta exitosamente", content = {
			@Content(mediaType = "application/json", schema = @Schema(implementation = com.foodfinder.dtos.ResponseDTO.class)) }),
			@ApiResponse(responseCode = "400", description = "La petición no puede ser entendida por el servidor debido a errores de sintaxis, el cliente no debe repetirla no sin antes hacer modificaciones", content = {
					@Content(mediaType = "application/json", schema = @Schema(implementation = ResponseDTO.class)) }),
	...
	@PutMapping("/{id}")
    public ResponseEntity<ResponseDTO> update(@PathVariable Integer id, @RequestBody MenuDTO menu) {
        return this.menuServiceImpl.update(id, menu);
    }
